...
export default function KitchenbaysPage() {
  const Kitchenbays = [
    { name: 'Ramesh Kumar', craft: 'Bidriware Kitchenbay', location: 'Karnataka', bio: 'A 4th generation Kitchenbay preserving the 500-year-old silver inlay craft of Bidar.', img: 'https://images.unsplash.com/photo-1532054992523-28f09d846065?q=80&w=600&auto=format&fit=crop' },
    ...
    <div className="min-h-screen flex flex-col bg-brand-bg font-sans">
      ...
      <main className="flex-1">
        <div className="bg-brand-card py-20 text-center px-4 border-b border-gray-100">
          ...
          <h1 className="font-serif text-5xl font-bold text-brand-text mb-6">Meet Our Kitchenbays</h1>
            ...
            {Kitchenbays.map((Kitchenbay, i) => (
              <div key={i} className="flex flex-col sm:flex-row gap-8 items-center sm:items-start group bg-brand-card/50 p-6 rounded-2xl border border-gray-100 hover:shadow-lg transition-shadow">
                <div className="w-40 h-40 rounded-full overflow-hidden shrink-0 shadow-lg relative">
                  <img src={Kitchenbay.img} alt={Kitchenbay.name} className="w-full h-full object-cover transition-transform duration-700 group-hover:scale-110 grayscale group-hover:grayscale-0" />
                  <div className="absolute inset-0 ring-1 ring-inset ring-black/10 rounded-full" />
                </div>
                <div className="text-center sm:text-left">
                  <h3 className="font-serif text-3xl font-bold text-brand-text mb-2">{Kitchenbay.name}</h3>
                  <p className="text-brand-accent font-medium mb-1">{Kitchenbay.craft}</p>
                  <p className="text-brand-muted text-xs mb-4 uppercase tracking-wider">{Kitchenbay.location}</p>
                  <p className="text-brand-text leading-relaxed mb-6">{Kitchenbay.bio}</p>
