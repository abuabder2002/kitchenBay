...
// Define tool schemas
const InteractiveBrowserActionSchema = zod_1.z.object({
    url: zod_1.z.string().optional().describe("URL to navigate to (if not already on the correct page)"),
    selector: zod_1.z.string().optional().describe("CSS selector of element to interact with"),
    action: zod_1.z.enum(["click", "fill", "verify_visible", "verify_text", "screenshot"]).describe("Action to perform"),
    value: zod_1.z.string().optional().describe("Value to fill in input (required if action is fill, or text to verify for verify_text)"),
});
const VerifyDbRecordSchema = zod_1.z.object({
    model: zod_1.z.string().describe("Prisma model name (e.g., 'product', 'order')"),
    where: zod_1.z.string().describe("JSON string representing the 'where' clause for prisma (e.g., '{\"name\":\"New Product\"}')"),
});
const RunE2eTestSchema = zod_1.z.object({
    testName: zod_1.z.enum(["add_product_flow"]).describe("Name of the predefined E2E test to run"),
});
    ...
    try {
        ...
        if (name === "analyze_project") {
            ...
            if (fs.existsSync(pkgPath))
                pkgInfo = fs.readFileSync(pkgPath, "utf-8");
            if (fs.existsSync(schemaPath))
                schemaInfo = fs.readFileSync(schemaPath, "utf-8");
            return {
                content: [{
                        type: "text",
        ...
        if (name === "interactive_browser_action") {
            ...
            switch (parsed.action) {
                ...
                case "verify_visible":
                    if (!parsed.selector)
                        throw new Error("Selector required for verify_visible");
                ...
                case "verify_text":
                    if (!parsed.selector || !parsed.value)
                        throw new Error("Selector and value required for verify_text");
